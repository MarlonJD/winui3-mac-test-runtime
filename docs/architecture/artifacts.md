...
- `macos-windowed-host.json`, `MacOsWindowedHost.swift`,
  `launch-macos-windowed.sh`, and `macos-windowed-events.jsonl`: generated by
  `winui3-mac-runner macos-windowed-host --artifacts <portable-scenario-dir>
  --output <dir> [--launch]` for local/manual macOS debugging. The generated
  AppKit host displays the Skia runtime PNG artifact, converts window points
  back into runtime coordinates, hit-tests against `tree.json`, and records
  mouse/key/scroll events as internal-driver evidence. This lane is
  `macos-windowed`, local/manual only, and intentionally does not implement the
  later AX adapter or raw Metal rendering path.
