...
| `portable-headless-comparison-dashboard.json` | `0.1` |
| `macos-windowed-host.json` | `0.1` |
| `macos-windowed-ax-adapter.json` | `0.1` |
| `windows-custom-runtime-uia-provider.json` | `0.1` |
| `windows-custom-runtime-uia-tree.json` | `0.1` |
...
- `native-reference-import.json`: normalized import manifest for downloaded
  ...
  references.
- `portable-headless-comparison-dashboard.json` and
  `portable-headless-comparison-dashboard.md`: generated by
  `winui3-mac-runner portable-headless-dashboard --portable <dir>
  --windows-reference <dir> --output <dir>`. The dashboard compares
  `portable-headless` scenario results, automation nodes, pattern/name/
  control-type metadata, bounds tolerance, and visual diff metrics against
  `windows-reference` native WinUI artifacts. Failures are reported as
  actionable diagnostics and do not imply OS-level UI automation coverage for
  the portable lane.
- `macos-windowed-host.json`, `MacOsWindowedHost.swift`,
  `launch-macos-windowed.sh`, `macos-windowed-events.jsonl`, and
  `macos-windowed-live-state.json`: generated by `winui3-mac-runner
  macos-windowed-host --artifacts <portable-scenario-dir> --output <dir>
  [--launch]` for local/manual macOS debugging. The generated AppKit host
  displays the Skia runtime PNG artifact, converts window points back into
  runtime coordinates, hit-tests against `tree.json`, records mouse/key/scroll
  events as internal-driver evidence, and maintains local live interaction state
  for focus, button press, checkbox/toggle, selection, and text input overlays.
  This lane is `macos-windowed`, local/manual only, and intentionally does not
  implement the later AX adapter or raw Metal rendering path.
- `macos-windowed-ax-adapter.json`, `macos-ax-tree.json`, and
  `MacOsAxAdapter.swift`: generated by
  `winui3-mac-runner macos-ax-adapter --automation <automation-core.json>
  --output <dir> [--scenario <name>]` for optional local/manual macOS AX
  validation. The adapter maps `AutomationCore` control types and patterns to
  `NSAccessibilityElement` roles/actions such as `AXButton`, `AXStaticText`,
  `AXTextField`, `AXCheckBox`, `AXRadioButton`, `AXList`, `AXRow`,
  `AXScrollArea`, `AXPress`, `AXValue`, `AXSelected`, and `AXScroll`. This lane
  is `macos-windowed-ax`, not portable-headless, not raw Metal rendering, and
  not default PR CI.
- `windows-custom-runtime-uia-provider.json`,
  `windows-custom-runtime-uia-tree.json`, and
  `WindowsCustomRuntimeUiaProvider.cs`: generated by
  `winui3-mac-runner windows-custom-runtime-uia --automation
  <automation-core.json> --output <dir> [--scenario <name>]` for the optional
  Windows custom-runtime UIA provider lane. The tree maps `AutomationCore`
  control types and patterns to Windows UIA `ControlType` and
  `InvokePattern`/`ValuePattern`/`TogglePattern`/`SelectionItemPattern`/
  `ScrollPattern` names, while the generated provider source sketches the
  `IRawElementProviderSimple`/fragment/pattern provider surface for a custom
  rendered Windows host. This lane is `windows-custom-runtime`, uses
  `flaui-uia3` against this runtime's provider, and must not be confused with
  `windows-reference` native WinUI evidence.
...
- `screenshots/mac-runtime.metadata.json`: portable-headless screenshot
  metadata sidecar for `skia-v2` PNG output, including mode, lane, driver,
  renderer, renderer version, viewport, scale, theme, platform, font profile,
  text measurement mode, and image integrity.
...
- `docs/visual-parity/broader-control-state-coverage.json`: generated Phase 14
  compatibility dashboard for ComboBox, ListView, InfoBar, Flyout,
  ContentDialog, Slider, ProgressRing, and ProgressBar. It explicitly separates
  `supported`, `partial`, and `planned` control support, names default/hover/
  pressed/disabled/focused/selected state coverage, and records VisualState,
  resource/theme, accessibility-pattern, scenario, and known-gap coverage
  without claiming native visual fidelity.
...
- `visual/windows-reference.json`: reference provenance copied from the Windows
  ...
  title, capture mode, and captured dimensions. Native WinUI reference captures
  from `.github/workflows/windows-native-screenshot.yml` also record
  `lane=windows-reference`, `runtime=native-winui`, `driver=flaui-uia3`, and
  `renderer=native-winui` so the artifacts cannot be confused with
  `portable-headless` or later `windows-custom-runtime` output.
