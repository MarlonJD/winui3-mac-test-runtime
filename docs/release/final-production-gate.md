...
| Gate | Evidence |
| --- | --- |
| PR #3 CI release-candidate evidence | Run `26969205766`, commit `3c19711`, passed build, tests, full strict scenario sweep, corpus ingestion, benchmark/flake, package dry run, release-check, and artifact uploads including `strict-scenario-sweep`. |
...
- Native reference readiness is tracked separately in
  `docs/visual-parity/native-reference-readiness.json`; rows that are
  runtime-bound, placeholder, unavailable, or offscreen remain release blockers.
...
Windows references are diagnostic context rather than final component-ready
native sources, especially in layout/media rows such as `Image`,
`AnimatedIcon`, `InkCanvas / InkToolbar`, `MediaPlayerElement`, `WebView2`,
title bar customization, and system backdrop/Mica.
