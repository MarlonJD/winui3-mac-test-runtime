...
| Level 6: Windows Reference Visual Compatibility | partial | Public `windows-latest` workflow captures Windows references, and local macOS strict runs compare against them with scenario-local pixel thresholds for shell, interaction/binding, control-gallery, public admin/workbench, and component parity lab strict fixture categories. Current checked-in examples include synthetic `WindowsNativeProbe` captures; native WinUI fixture references must be inspected before parity claims. Whole-screenshot passes do not override component-level grades, and text-only component output remains `not-rendered`. |
...
| Windows reference source of truth | partial | Public `windows-latest` workflow captures reference screenshots, while local macOS strict runs produce comparison artifacts. Checked-in examples still include synthetic `WindowsNativeProbe` evidence; native WinUI fixture capture and provenance inspection are required before production visual claims. |
