...
| `component-evidence.json` | supported | Versioned component lab evidence captures catalog status, presence, interaction status, visual grade, effective per-component thresholds, target layout region, crop paths, blank-crop status, known gaps, and optional reference or crop diff metrics. It is the source of truth for marking planned, unsupported, text-only, or absent macOS component output as `not-rendered`. |
