...
| Windows reference source of truth | partial | Public `windows-latest` workflow captures generic reference screenshots, while local macOS strict runs produce comparison artifacts. Current references include synthetic `WindowsNativeProbe`; native WinUI fixture capture is required before production visual claims. |
