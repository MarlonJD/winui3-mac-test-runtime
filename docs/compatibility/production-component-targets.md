...
| Theme dictionaries and resources | `XamlControlsResources`, `ResourceDictionary.ThemeDictionaries`, `Color`, `SolidColorBrush`, `CornerRadius` | partial for static `XamlControlsResources`, theme dictionaries, `Color`, `SolidColorBrush`, and `CornerRadius` diagnostics | `component-layout-media-light`, `component-layout-media-dark`, `component-layout-media-high-contrast` | strict missing-resource diagnostics | visible themed text and non-color-only state in light, dark, high contrast | `usable` when claimed | theme/scale E2E flow | Full Fluent resource dictionaries, dynamic invalidation, exact icon/resource metrics, and complete typed brush/color conversion are incomplete. |
| Adaptive layout containers | `Expander`, annotated scrollbar pattern, `SemanticZoom`, `SplitView`, `TwoPaneView` | partial static diagnostics | `component-layout-media-light` | static expanded/open/two-pane evidence only | exported names, bounds, expanded/open state where modeled | `usable` when claimed | layout container smoke evidence | Collapse, zoom switching, scroll annotation behavior, display modes, responsive breakpoints, keyboarding, and automation remain incomplete. |
| Keyboard, accessibility, and UI automation | focus traversal, accelerators, selected/checked/value states, FlaUI-style automation identity, bounds, action dispatch, and screenshot capture | partial | all harness-ring scenarios plus native Windows automation reference runs | tab order, accelerator invocation, keyboard selection, focus assertion, FlaUI 5.0 + FlaUI.UIA3 native Windows smoke, macOS adapter smoke when available | Narrator-ready names, roles, state, value, relationships, and UIA-compatible automation metadata | n/a | every harness E2E flow | Focus visuals, broad keyboard routing, full UIA automation peers, and macOS FlaUI-compatible adapter coverage are incomplete. |

## Ring 2 And Explicit Non-Goals

| Area | Components or APIs | Production status |
| --- | --- | --- |
| Advanced controls | `TabView`, `TreeView`, `BreadcrumbBar`, `RatingControl`, `PersonPicture`, `ColorPicker`, date/time controls | Ring 2 unless a public production target moves a component into Ring 1. |
